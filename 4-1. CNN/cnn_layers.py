    ...
    for i in range(0, H_prime):
        for j in range(0, W_prime):
            out[:, :, i, j] = \
                np.sum( \
                    np.sum( \
                        np.sum(x[:, np.newaxis, :, i:i+HH, j:j+WW] * w, axis=-1 \
                              ), axis=-1 \
                    ), axis=-1 \
                )
    ...
    for i in range(0, H):
        for j in range(0, W):
            dx[:, :, i, j] = \
                np.sum( \
                    np.sum( \
                        np.sum(w_pad[:, :, i:i+H_prime, j:j+W_prime] * dout[:,:, np.newaxis,::-1,::-1], axis=-1 \
                              ), axis=-1 \
                    ), axis=1 \
                )
    # dL/dw   
    for i in range(0, HH):
        for j in range(0, WW):
            dw[:, :, i, j] = \
                np.sum( \
                    np.sum( \
                        np.sum(x[:, np.newaxis, :, i:i+H_prime, j:j+W_prime] * dout[:, :, np.newaxis, :, :], axis=-1 \
                              ), axis=-1 \
                    ), axis=0 \
                )
